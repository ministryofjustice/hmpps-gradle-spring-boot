...
class DependencyManagementPluginManagerTomcatTest : GradleBuildTest() {
  ...
  fun `Wrong transitive version of tomcat should be overridden by the plugin`(projectDetails: ProjectDetails) {
    ...
    assertThat(jarContainsTomcatEmbedCore(jarFile, "11.0.20")).isFalse
    assertThat(jarContainsTomcatEmbedCore(jarFile, "11.0.21")).isTrue
    assertThat(jarContainsTomcatEmbedWebsocket(jarFile, "11.0.20")).isFalse
    assertThat(jarContainsTomcatEmbedWebsocket(jarFile, "11.0.21")).isTrue
  }
}
